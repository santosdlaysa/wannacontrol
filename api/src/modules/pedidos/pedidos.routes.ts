...
router.post('/', authorize(Perfil.GARCOM, Perfil.ADMIN, Perfil.GERENTE), validate(createPedidoSchema), asyncHandler(pedidosController.criar));
...
router.patch('/:id/cancelar', authorize(Perfil.GERENTE, Perfil.ADMIN), asyncHandler(pedidosController.cancelar));
