...
export default function DashboardLayout({
  ...
  const isSuperAdmin = !!user && !restaurante;
  const [assinaturaAtiva, setAssinaturaAtiva] = useState<boolean>(() => {
  ...
  useEffect(() => {
    ...
    // Super admin nao tem restaurante — acesso sempre liberado
    if (!restaurante) {
        ...
        {!isKitchenFullscreen && (
          <aside className="w-64 bg-cafe-900 text-white flex flex-col fixed h-full z-30">
            ...
            <nav className="flex-1 py-4 px-3 space-y-1 overflow-y-auto scrollbar-hide">
              {isSuperAdmin && (
                <Link
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors duration-150 ${
                    pathname === '/admin' ? 'bg-cafe-700 text-cafe-50' : 'text-cafe-200 hover:bg-cafe-800 hover:text-white'
                  }`}
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {!isSuperAdmin && filteredNav.map((item) => {
